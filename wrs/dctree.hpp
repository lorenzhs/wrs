...
public:
    ...
    dctree()
        : weights_(nullptr), cache_(cache_size), num_leaves_(-1),
          num_inners_(-1), size_(-1), levels_(-1) {}
    ...
    std::pair<size_type, double> get_leaf(size_type leaf_id) const {
        assert(leaf_id < num_leaves_);
        return std::make_pair(ids_[leaf_id], weights_[num_inners_ + leaf_id + 1]);
    }
    ...
    void assign(size_t count, Callback &&callback, StochasticLib1 &stoc) const {
        sLOG << "Assigning" << count << "samples in dctree of" << num_inners_
             << "inner nodes and" << num_leaves_ << "leaves";
    ...
    friend std::ostream &operator<<(std::ostream &os, const dctree &t) {
        ...
        tlx::LoggerFormatter<std::vector<double>>::print(
            os, std::vector<double>(t.weights_.get(),
                                    t.weights_.get() +
                                        std::min<size_type>(t.size_, 100)));
        os << ", ids = ";
...
protected:
    ...
    template <typename Callback, typename RNG>
    void assign_one(size_type min_id, size_type max_id, Callback &&callback,
                    RNG &rng) const {
        ...
        do {
            ...
            id += min_id;
            sLOG << "Rejection sampling: trying item" << id + 1 - min_id << "of"
                 << range << "item weight" << weights_[id] << "rand=" << rand
                 << "max_weight=" << max_weight_
                 << (rand <= weights_[id] ? "ACCEPTED" : "REJECTED") << "leaf"
                 << leaf_id(id);
    ...
    void assign_few(size_t count, size_type min_id, size_type max_id,
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            do {
                ...
                sLOG << "Rejection sampling: trying item" << id + 1 - min_id
                     << "of" << range << "item weight" << weights_[id]
                     << "rand=" << rand << "max_weight=" << max_weight_
    ...
    void assign_recursive(size_t count, size_type index, Callback &&callback,
                          StochasticLib1 &stoc, RNG &rng) const {
        ...
        if (left_w < 1e-10) {
            ...
            if (is_leaf(right_id))
                callback(ids_[leaf_id(right_id)], count);
            else
                assign_recursive<early_abort>(count, right_id, callback, stoc, rng);
            return;
        } else if (right_w < 1e-10) {
            sLOG << "\tright subtree of" << index << "is empty, giving all"
                 << count << "samples to" << left_id << "of weight" << left_w;
            if (is_leaf(left_id))
                callback(ids_[leaf_id(left_id)], count);
            else
                assign_recursive<early_abort>(count, left_id, callback, stoc, rng);
            return;
        }
        ...
        if (num_left > 0)
            assign_recursive<early_abort>(num_left, left_id, callback, stoc, rng);
        if (num_left < count)
            assign_recursive<early_abort>(count - num_left, right_id, callback,
                                          stoc, rng);
    }
    ...
    static constexpr size_type left(size_type index) {
        return 2 * index;
    }
    static constexpr size_type right(size_type index) {
        return 2 * index + 1;
    }
    size_type leaf_id(size_type index) const {
        return index - num_inners_ - 1;
    }
    bool is_leaf(size_type index) const {
        return index > num_inners_;
    }
};
