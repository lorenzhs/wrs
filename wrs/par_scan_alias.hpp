...
public:
    ...
    void construct(Iterator begin, Iterator end) {
        ...
        timers_.push_back(t.get_and_reset());
        LOGC(time) << "Step 1: classification & table init took "
                   << timers_.back() << "ms";
        ...
        parallel_do_range(
            [&](size_type min, size_type max, int /* thread */) {
                ...
                for (index = min; index < loopmax; index += 2) {
                    size_type nexti = index + 1, curr = classifier_[index],
                              next = classifier_[nexti], pos1, pos2;
                    if (curr == prev) {
        ...
        subproblem problem = {
            0,          num_light_,         num_light_, size_, size_,
            num_light_, size_ - num_light_, 0.0,        0.0,   false,
            false};
